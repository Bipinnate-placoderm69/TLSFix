/*
 * builtins.c — integer-division compiler-rt builtins for the 32-bit ARM slices (armv6 + armv7).
 *
 * Two reasons the armv6 slice needs these compiled in:
 *   1. iOS 2-4's libSystem.B.dylib doesn't export the compiler-rt builtins (___udivdi3 etc.),
 *      and we link with -undefined dynamic_lookup, so they'd otherwise be resolved at runtime
 *      and abort the host (the crash that broke MobileSafari on iOS 3.2.2 was exactly ___udivdi3).
 *   2. Xcode's libclang_rt.ios.a has no armv6 architecture to force-load (armv7/arm64 do).
 *
 * ARM11 (armv6) also has no hardware integer divide, so the 32-bit ___divsi3/___modsi3 family is
 * needed too. Plain binary long division — correct, and fine for the slow 2G/3G-class devices.
 */
...
static du_int udivmod64(du_int a, du_int b, du_int *rem) {
    if (b == 0) { if (rem) *rem = 0; return 0; }   /* avoid trap on /0; UB anyway */
